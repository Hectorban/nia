defmodule NiaWeb.ContentEditor do
  @moduledoc false
  ...
  def content_field(%{field: %Phoenix.HTML.FormField{} = field} = assigns) do
    ...
    |> assign(
      :errors,
      Enum.map(field.errors, &NiaWeb.CoreComponents.translate_error(&1))
    )
  ...
  defp block(%{block: %{type: "niaImage", data: %{url: url, caption: caption}}} = assigns) do
    assigns =
      assigns
      |> assign(:url, url)
      |> assign(:caption, caption)
